"""tool_recall_test.py - Tool Recall Capacity Stress Test (LM Studio)

Purpose:
    Measures how many tools an AI model can reliably remember and correctly
    select from. Progressively increases the number of available tools from
    2 up to 30 and records accuracy at each level. Reveals the model's
    effective "tool memory" ceiling.

    Uses a pool of 30 diverse tools spanning weather, email, translation,
    navigation, finance, media, system admin, and more.

Sequence:
    1. First verify basic tool calling works with showdown_lmstudio.py
    2. Then test multi-step chaining with showdown_media_tools.py
    3. Run THIS script last to stress-test how many tools the model can
       handle simultaneously.

Prerequisites:
    - LM Studio running with at least one model loaded
    - Default server endpoint: http://localhost:1234/v1

Dependencies: Python stdlib only (works on Windows and macOS)

Usage:
    python tool_recall_test.py         # GUI mode (tkinter)
    python tool_recall_test.py --cli   # terminal mode
"""
...
def _get_log_path() -> str:
    ...
    return f"lms_tool_recall_{ts}.log"
...
# --- Large pool of diverse mock tools ---
# Each entry: (name, description, parameters, test_prompt, expected_args_key)
...
TOOL_POOL = [
    {
        ...
        "mock_result": "Berlin: 18Â°C, partly cloudy, humidity 62%.",
    },
    {
        "name": "send_email",
    ...
    },
    {
        ...
        "mock_result": "Translation: ãŠã¯ã‚ˆã†ã”ã–ã„ã¾ã™ã€ãŠå…ƒæ°—ã§ã™ã‹ï¼Ÿ",
    },
    {
        "name": "set_timer",
...
def list_models() -> list:
    """Fetch currently-loaded models from LM Studio.

    Prefers LM Studio's native REST API (/api/v0/models) which exposes a
    `state` field so we can filter to only `loaded` models. This avoids
    iterating over every model in the user's library (which would trigger
    JIT load/unload swaps and make the test appear to run endlessly).

    Falls back to the OpenAI-compatible /v1/models endpoint if the native
    API is unavailable; in that case all returned models are tested.
    """
    # Derive the native API base by stripping a trailing "/v1" if present.
    base = LMSTUDIO_BASE_URL.rstrip("/")
    native_base = base[:-3] if base.endswith("/v1") else base
    native_url = f"{native_base}/api/v0/models"
    try:
        req = urllib.request.Request(native_url, method="GET")
        with urllib.request.urlopen(req, timeout=10) as resp:
            body = json.loads(resp.read().decode("utf-8"))
        entries = body.get("data", [])
        loaded = [m["id"] for m in entries if m.get("state") == "loaded"]
        if loaded:
            return loaded
        # Native API reachable but nothing loaded: return empty so caller can warn.
        if entries:
            return []
    except Exception:
        pass

    # Fallback: OpenAI-compatible endpoint (no load-state info).
    url = f"{base}/models"
...
def test_single_call(model: str, target_tool: dict, all_tool_schemas: list) -> dict:
    """
    Test if the model can select the correct tool from the available set.
    Returns result dict with correctness info.
    """
    ...
    try:
        response = lmstudio_chat(model, [{"role": "user", "content": prompt}], all_tool_schemas)
        ...
        return {"correct": correct, "called": called_name, "expected": expected_name, "elapsed": elapsed, "error": None}
...
def run_capacity_test(model: str, max_tools: int = None, log_callback=None) -> list:
    """
    Progressively test the model with increasing numbers of tools.
    Returns list of (tool_count, accuracy, details) tuples.
    """
    ...
    while level <= max_tools:
        ...
        if level < 5:
            ...
            level += 5
    if test_levels[-1] != max_tools and max_tools > test_levels[-1]:
    ...
    for tool_count in test_levels:
        ...
        # Test each tool in the subset (pick up to 5 to keep runtime reasonable)
        ...
        for target in test_targets:
            result = test_single_call(model, target, schemas)
...
class ToolRecallGUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Tool Recall Capacity Test (LM Studio)")

        # Detect screen resolution
        screen_w = self.root.winfo_screenwidth()
        screen_h = self.root.winfo_screenheight()
        win_w = min(950, int(screen_w * 0.8))
    ...
    def _build_ui(self):
        ...
        ttk.Label(ctrl, text="LM Studio URL:").grid(row=0, column=0, sticky=tk.W)
        self.url_var = tk.StringVar(value=LMSTUDIO_BASE_URL)
        ttk.Entry(ctrl, textvariable=self.url_var, width=35).grid(row=0, column=1, padx=5)

        ttk.Label(ctrl, text="Max tools:").grid(row=0, column=2, padx=(10, 0))
        ...
        phase_frame.grid(row=1, column=1, columnspan=4, sticky=tk.W, pady=(8, 0))
        ...
        ttk.Label(
            info_frame,
            text=f"Tool pool: {len(TOOL_POOL)} diverse tools. Tests model accuracy as tool count increases.",
            foreground="gray",
        ).pack(anchor=tk.W)
    ...
    def _start(self):
        ...
        global LMSTUDIO_BASE_URL
        LMSTUDIO_BASE_URL = self.url_var.get().strip().rstrip("/")
        ...
        threading.Thread(target=self._run, args=(max_t, phase), daemon=True).start()

    def _run(self, max_tools: int, phase: str = "both"):
        self._set_status("Connecting...")
        self._log("Connecting to LM Studio...")
        ...
        if not models:
            self._log("No models are currently loaded in LM Studio. Load one or more models in the LM Studio UI and try again.")
            self._finalize_log()
            self._finish()
            return
    ...
    def _render_chart(self, model: str, results: list):
        """Render a text-based bar chart of accuracy vs tool count."""
        lines = [f"Model: {model}", ""]
        lines.append("Tools | Accuracy | Bar")
        lines.append("------+----------+" + "-" * 32)
        for r in results:
            bar_len = int(r["accuracy"] * 30)
            bar = "#" * bar_len + "." * (30 - bar_len)
            lines.append(f"  {r['tool_count']:>3} | {r['accuracy']:>6.0%}   | [{bar}]")
        lines.append("")
...
def run_phase2_test(model: str, log_callback=None) -> dict:
    """
    Run Phase 2 adversarial tests against LM Studio.
    ...
    for i, tc in enumerate(PHASE2_TEST_CASES, 1):
        ...
        try:
            response = lmstudio_chat(
                model, [{"role": "user", "content": prompt}], schemas
            )
            elapsed = round(time.perf_counter() - start, 3)
...
def main_cli():
    ...
    log_and_print("Tool Recall Capacity Test (LM Studio)")
    ...
    try:
        ...
        log_and_print(f"ERROR: Cannot connect to LM Studio at {LMSTUDIO_BASE_URL}: {e}")
        _write_log(log_path, log_lines)
        return

    if not models:
        log_and_print("No models are currently loaded in LM Studio. Load one or more models in the LM Studio UI and try again.")
        _write_log(log_path, log_lines)
        return

    log_and_print(f"Models: {', '.join(models)}\n")
    ...
    for model in models:
        ...
        if not skip_phase1:
            results = run_capacity_test(model, log_callback=log_and_print)
        ...
        if not skip_phase2:
            run_phase2_test(model, log_callback=log_and_print)
