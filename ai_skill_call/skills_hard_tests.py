...
SKILL_REFUND_APPROVER = """# Skill: Refund Approval Engine
...
SKILL_PII_TRANSFORMER = """# Skill: PII Data Transformer
...
def eval_refund_approver(raw_output: str, thinking_enabled: bool) -> Tuple[bool, str]:
    output = _extract_final_output(raw_output, thinking_enabled).strip()
    fenced = False
    if output.startswith("```"):
        fenced = True
    ...
    expected = {
        1: {"decision": "APPROVED", "payout_amount": 50.00},
        2: {"decision": "APPROVED", "payout_amount": 100.00},
    ...
    for item in data:
        ...
        if oid not in expected:
            return False, f"Invalid or missing order_id: {oid}"
        exp = expected[oid]
        if item.get("decision") != exp["decision"]:
            return False, f"Order {oid}: Expected decision {exp['decision']}, got {item.get('decision')}"
        try:
            payout = float(item.get("payout_amount", -1))
            if abs(payout - exp["payout_amount"]) > 0.01:
                return False, f"Order {oid}: Expected payout {exp['payout_amount']}, got {payout}"
        except (TypeError, ValueError):
            return False, f"Order {oid}: Invalid payout_amount type"
        if not isinstance(item.get("reason"), str) or not item.get("reason"):
            return False, f"Order {oid}: Missing or invalid reason"

    return True, "OK"

def eval_pii_transformer(raw_output: str, thinking_enabled: bool) -> Tuple[bool, str]:
    output = _extract_final_output(raw_output, thinking_enabled).strip()
    fenced = False
    if output.startswith("```"):
        fenced = True
    ...
    forbidden = re.findall(r"\b(redacted|masked|protected|sanitized)\b", output, re.IGNORECASE)
    if forbidden:
        return False, f"Forbidden word(s): {forbidden}"
    if fenced:
        return False, "Output was wrapped in markdown code fences."
    ...
    expected_keys = {"emails", "phones", "credit_cards", "ssns", "generic_numbers"}
    ...
    gen_nums = data.get("generic_numbers", [])
    if "500 units" not in gen_nums or "1200 units" not in gen_nums:
        return False, f"Generic numbers transformation failed. Got: {gen_nums}"
...
SCENARIOS: List[Dict] = [
    ...
    {
        "name": "Moderate Skill 1: Code Refactorer",
    ...
    {
        "name": "Moderate Skill 2: Log Analyzer",
    ...
    {
        "name": "Complex Skill 1: Refund Approval",
        "skill_filename": "skill_refund_approver.md",
        "skill_content": SKILL_REFUND_APPROVER,
        "test_input": INPUT_REFUND_APPROVER,
        "evaluator": eval_refund_approver,
    },
    {
        "name": "Complex Skill 2: PII Data Transformer",
        "skill_filename": "skill_pii_transformer.md",
        "skill_content": SKILL_PII_TRANSFORMER,
        "test_input": INPUT_PII_TRANSFORMER,
        "evaluator": eval_pii_transformer,
    },
]
        ...
        try:
            ...
            for s_idx, scenario in enumerate(SCENARIOS, 1):
                ...
                for i in range(1, self.iterations + 1):
                    ...
                    self.progress(f"[Scenario {s_idx}/{len(SCENARIOS)}] Iteration {i}/{self.iterations}...")
                    ...
                    try:
                        ...
                        # Pre-evaluation check for leaked thinking tags
                        if not self.thinking_mode and re.search(r"<(?:think|thinking)>", raw_output, re.IGNORECASE):
                            passed = False
                        ...
                        if passed:
                            ...
                            fails += 1
                            self.log(f"    FAIL  ({latency:.2f}s): {reason}", "fail")
                ...
                total = passes + fails
                consistency = (passes / total * 100.0) if total > 0 else 0.0
            ...
            total_runs = sum(s["pass_count"] + s["fail_count"] for s in scenario_results)
            overall_consistency = round((total_passes / total_runs * 100.0), 2) if total_runs > 0 else 0.0
            ...
            (self.output_dir / "raw_outputs.json").write_text(json.dumps(raw_outputs, indent=2, ensure_ascii=False), encoding="utf-8")
            (self.output_dir / "summary_report.json").write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")

            self.log(f"\nTest suite complete. Overall consistency: {overall_consistency}%", "info")
            self.log(f"Reports saved to: {self.output_dir}", "info")
            self.complete(True, f"Done. Overall consistency: {overall_consistency}%")
...
class App(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(conn, text="Connection", font=ctk.CTkFont(size=14, weight="bold")).grid(row=0, column=0, columnspan=2, padx=10, pady=(8, 4), sticky="w")

        ctk.CTkLabel(conn, text="Base URL:").grid(row=1, column=0, padx=10, pady=4, sticky="w")
        self.url_var = ctk.StringVar(value="http://localhost:1234/v1")
        ctk.CTkEntry(conn, textvariable=self.url_var, width=280).grid(row=1, column=1, padx=10, pady=4, sticky="ew")
        ...
        self.test_conn_btn.grid(row=3, column=0, columnspan=2, padx=10, pady=(4, 10), sticky="ew")
        ...
        ctk.CTkEntry(cfg, textvariable=self.iter_var, width=80).grid(row=1, column=1, padx=10, pady=4, sticky="w")

        ctk.CTkLabel(cfg, text="Temperature:").grid(row=2, column=0, padx=10, pady=4, sticky="w")
        self.temp_var = ctk.DoubleVar(value=0.2)
        self.temp_slider = ctk.CTkSlider(cfg, from_=0.0, to=1.0, variable=self.temp_var, command=self._on_temp_change)
        ...
        ctk.CTkLabel(exec_frame, text="Execution", font=ctk.CTkFont(size=14, weight="bold")).grid(row=0, column=0, padx=10, pady=(8, 4), sticky="w")

        self.outdir_var = ctk.StringVar(value=str(self.base_dir / f"test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}"))
        ctk.CTkEntry(exec_frame, textvariable=self.outdir_var).grid(row=1, column=0, columnspan=2, padx=10, pady=4, sticky="ew")
        ctk.CTkButton(exec_frame, text="Browse...", command=self._on_pick_dir).grid(row=2, column=0, padx=10, pady=4, sticky="w")

        self.run_btn = ctk.CTkButton(exec_frame, text="Run Tests", fg_color="green", command=self._on_run)
        self.run_btn.grid(row=2, column=1, padx=10, pady=4, sticky="e")

        self.stop_btn = ctk.CTkButton(exec_frame, text="Stop", fg_color="darkred", state="disabled", command=self._on_stop)
        ...
        ctk.CTkLabel(prog_frame, textvariable=self.progress_var, font=ctk.CTkFont(size=12, weight="bold")).pack(padx=10, pady=6, anchor="w")
        ...
        self.log_text = Text(log_frame, wrap=WORD, bg="#1e1e1e", fg="#d4d4d4", insertbackground="#d4d4d4", font=("Consolas", 11), bd=0)
    ...
    def _on_pick_dir(self):
        ...
        if d:
            ...
            new_dir = self.base_dir / f"test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def _on_run(self):
        ...
        if not outdir.name.startswith("test_results_"):
            outdir = outdir / f"test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            self.outdir_var.set(str(outdir))
        if outdir.exists():
            parent = outdir.parent
            outdir = parent / f"test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            self.outdir_var.set(str(outdir))
    ...
    def _on_close(self):
        if self.runner_thread and self.runner_thread.is_alive():
            resp = messagebox.askyesno("Tests Running", "Tests are currently running. Aborting will save partial results and quit. \n\nDo you want to abort and exit?")
            if resp:
