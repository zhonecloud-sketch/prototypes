...
function getTutorialForNews(newsItem) {
    ...
    // ========== Check for FOMO Rally news ==========
    // All FOMO Rally hints are generated by getFOMORallyTutorialHint() in fomoRally.js
    if (newsItem.isFOMO || newsItem.isFOMORally || newsItem.newsType === 'fomo' || newsItem.newsType === 'fomo_rally') {
        if (typeof getFOMORallyTutorialHint === 'function') {
            const fomoHint = getFOMORallyTutorialHint(newsItem);
            if (fomoHint && fomoHint.type) {
                return fomoHint;
            }
        }
        // Fallback to legacy TUTORIAL_HINTS if module not available
    ...
    // All Short Squeeze hints are generated by getShortSqueezeTutorialHint() in shortSqueeze.js
    if (newsItem.isShortSqueeze || newsItem.newsType === 'short_squeeze') {
        if (typeof getShortSqueezeTutorialHint === 'function') {
            const squeezeHint = getShortSqueezeTutorialHint(newsItem);
            if (squeezeHint && squeezeHint.type) {
                return squeezeHint;
            }
        }
        // Fallback to legacy TUTORIAL_HINTS if module not available
    ...
    // Includes: crash, dead_cat_bounce, reversal_forming, reversal_confirmed, crash_resolution, recovery, recovery_complete
    const dcbNewsTypes = ['crash', 'dead_cat_bounce', 'reversal_forming', 'reversal_confirmed', 'crash_resolution', 'recovery', 'recovery_complete'];
    if (newsItem.isDeadCatBounce || newsItem.isCrashResolution || newsItem.isRecovery || dcbNewsTypes.includes(newsItem.newsType)) {
    ...
    // Includes: short_report, short_report_update, rebuttal, base_update, breakout, breakdown, short_report_resolution
    // Also handles insider_buying during SSR (isInsiderBuying flag)
    const ssrNewsTypes = ['short_report', 'short_report_update', 'rebuttal', 'base_update', 'breakout', 'breakdown', 'short_report_resolution'];
    if (newsItem.isShortReport || newsItem.isInsiderBuying || ssrNewsTypes.includes(newsItem.newsType)) {
    ...
    // All Liquidity Sweep hints are generated by getLiquiditySweepTutorialHint() in liquiditySweep.js
    if (newsItem.type === 'liquidity_sweep' || newsItem.newsType === 'liquidity_sweep') {
        if (typeof getLiquiditySweepTutorialHint === 'function') {
            const sweepHint = getLiquiditySweepTutorialHint(newsItem);
            if (sweepHint && sweepHint.type) {
                return sweepHint;
            }
        }
        // No fallback needed - module handles all phases
        return null;
    }

    // ========== News Shakeout hints ==========
    // All News Shakeout hints are generated by getNewsShakeoutTutorialHint() in newsShakeout.js
    if (newsItem.type === 'news_shakeout' || newsItem.newsType === 'news_shakeout') {
        if (typeof getNewsShakeoutTutorialHint === 'function') {
            const shakeoutHint = getNewsShakeoutTutorialHint(newsItem);
            if (shakeoutHint && shakeoutHint.type) {
                return shakeoutHint;
            }
        }
        // No fallback needed - module handles all phases
    ...
    // All Executive Change hints are generated by getExecutiveChangeTutorialHint() in executiveChange.js
    if (newsItem.isExecutiveChange || newsItem.newsType === 'executive_change') {
        if (typeof getExecutiveChangeTutorialHint === 'function') {
            const execHint = getExecutiveChangeTutorialHint(newsItem);
            if (execHint && execHint.type) {
                return execHint;
            }
        }
        // Fallback to legacy inline hints
    ...
    // All Strategic Pivot hints are generated by getStrategicPivotTutorialHint() in strategicPivot.js
    if (newsItem.isStrategicPivot || newsItem.newsType === 'strategic_pivot') {
        if (typeof getStrategicPivotTutorialHint === 'function') {
            const pivotHint = getStrategicPivotTutorialHint(newsItem);
            if (pivotHint && pivotHint.type) {
                return pivotHint;
            }
        }
        // No fallback needed - module handles all types
        return null;
    }
